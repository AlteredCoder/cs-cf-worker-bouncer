...
const handleTurnstilePost = async (request, body, turnstile_secret) => {
  ...
  if (!outcome.success) {
    ...
    newResponse.headers.set("Set-Cookie", `captchaAuth=${jwtToken}; Path=/; HttpOnly; Secure; SameSite=Strict;`)
...
export default {
  async fetch(request, env, ctx) {
    ...
    const doCaptcha = async (env, zoneForThisRequest) => {
      ...
      if (cookie["captchaAuth"] !== undefined) {
        console.log("captchaAuth cookie is present")
        // Check if the JWT token is valid
        try {
          const decoded = await jwt.verify(cookie["captchaAuth"], turnstileCfg["secret"]);
      ...
      if (request.method === "POST") {
        ...
        if (formBody.get('cf-turnstile-response')) {
          ...
          return await handleTurnstilePost(request, formBody, turnstileCfg["secret"])
        }
      }
